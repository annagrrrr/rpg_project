...
{
    public HealthManager healthManager;  // опять очень много public полей
    ...
    private void HandleAttack() // Почему контроллег игрока оперирует кнопками мыши? надо выделить отдельный сервис ввода
    {
    ...
    public void DealDamage(Enemy target, int damage, DamageType type)
    {
        ...
        {
            meleeWeapon.PerformAttack(); // а где target?
        }
